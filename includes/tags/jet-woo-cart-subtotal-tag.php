...
class Jet_Woo_Card_Subtotal_Tag extends \Elementor\Core\DynamicTags\Tag {

	public function get_name() {
		return 'jet-woo-cart-subtotal-tag';
	}
	...
	public function render() {
		if ( WC()->cart && ! is_null( WC()->cart ) ) {
			...
			switch ( $format ) {
				case 'html':
					?>
					<span class="jet_woo_cart_subtotal_tag"><?php echo wp_kses_post( WC()->cart->get_cart_subtotal() ); ?></span>
					<?php
					break;

				case 'plain':
				default:
					echo esc_html( WC()->cart->get_subtotal() );
					break;
			}
		} else {
